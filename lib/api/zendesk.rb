...
module Zendesk
    ...
    def self.contact_ticket(lead)
        ZendeskAPI::Ticket.create(@@client, :subject => "#{lead.full_name} from #{lead.business_name}", :descrpition => "Create Ticket", :comment => { :value => "The contact #{lead.full_name} can be reached at email #{lead.email} and at phone number #{lead.phone_number}. #{lead.building_type} has a project named #{lead.project_description} which would require contribution from Rocket Elevators.\n#{lead.project_description}\nAttached Message: #{lead.message} The Contact uploaded an attachment."}, :submitter_id => @@client.current_user.id, :type => "question", :priority => "urgent")
    end
    ...
    def self.intervention_ticket(serial_number, address, full_name_tech_person)
        ZendeskAPI::Ticket.create(@@client, :subject => "Elevator #{serial_number} from #{address}", :description => "Create Ticket", :comment => { :value => "Hello CEO, the elevator #{serial_number} has been set to Intervention. #{full_name_tech_person} has been notified."}, :submitter_id => @@client.current_user.id, :type => "notification", :priority => "none")
    end
end